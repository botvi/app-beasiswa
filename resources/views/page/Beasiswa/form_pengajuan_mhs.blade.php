        ...
        <div class="col-lg-8 grid-margin stretch-card">
            <div class="card shadow-lg">
                <div class="card-body">
                    ...
                    <form action="{{ route('pendaftaran_beasiswa.store') }}" enctype="multipart/form-data" method="POST">
                        @csrf
                        <input name="mahasiswa_id" type="hidden" value="{{ $mhs->id ?? '' }}">
                        <div class="form-group">
                            <label for="mahasiswa_id">Mahasiswa</label>
                            <input class="form-control" disabled type="text" value="{{ $mhs->nama ?? '' }}">
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control @error('beasiswa_id') is-invalid @enderror" id="beasiswa_id"
                                ...
                                @foreach ($beasiswaList as $beasiswa)
                                    <option value="{{ $beasiswa->id }}">{{ $beasiswa->judul }}</option>
                                @endforeach
                            </select>
                            @error('beasiswa_id')
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" name="keterangan"
                                placeholder="Masukkan Keterangan" required rows="4"></textarea>
                            @error('keterangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input class="form-control-file @error('dokumen') is-invalid @enderror" id="dokumen"
                                name="dokumen" required type="file">
                            @error('dokumen')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <button class="btn btn-primary bg-blue-500" type="submit">Ajukan Beasiswa</button>
                    </form>
                </div>
            </div>
        </div>
...
@section('script')
    ...
    <script>
        $("#beasiswa_id").change(function() {
            const reqFunc = async () => {
                const readed = await axios.get(`/beasiswa/${$(this).val()}/id`).catch(() => {
                    $("#info-beasiswa").html(``)
                })
                if (readed) {
                    const {
